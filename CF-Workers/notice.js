...
export function generateNoticeScript(clientVersion) {
  ...
  script += '    banner.innerHTML = "📢 有新版本，建议升级 <a href=\\"' + UPDATE_CONFIG.downloadUrl + '\\" target=\\"_blank\\" style=\\"color: white; text-decoration: underline; font-weight: 600;\\">立即下载</a> <span style=\\"cursor: pointer; margin-left: 8px; opacity: 0.8;\\">✕</span>";\n';
  script += '\n';
  script += '    const closeBtn = banner.querySelector("span");\n';
  ...
  script += '\n';
  script += '    const button = document.createElement("a");\n';
  script += '    button.href = "' + UPDATE_CONFIG.downloadUrl + '";\n';
  script += '    button.target = "_blank";\n';
  script += '    button.style.cssText = "display: inline-block; background: #2563eb; color: white; padding: 12px 32px; border-radius: 8px; text-decoration: none; font-size: 16px; font-weight: 600; transition: background 0.2s;";\n';
  script += '    button.textContent = "立即下载";\n';
  script += '    button.onmouseover = function() { this.style.background = "#1d4ed8"; };\n';
  script += '    button.onmouseout = function() { this.style.background = "#2563eb"; };\n';
  script += '\n';
